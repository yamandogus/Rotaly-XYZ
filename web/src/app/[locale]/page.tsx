...
export default function HomePage() {
  ...
  const categories = [
    ...
    { name: t("categoryBungalow"), label: "Bungalov", icon: "/icons/bungalov.svg" },
    { name: t("categoryRoom"), label: "Oda", icon: "/icons/oda.svg" },
    { name: t("categoryCountryHouse"), label: "Tatil Köyü", icon: "/icons/tatilköyü.svg" },
    { name: t("categoryPension"), label: "Pansiyon", icon: "/icons/pansiyon.svg" },
...
<p className="text-base md:text-lg max-w-2xl mb-8 text-gray-400 font-medium">
  ...
  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4">
    {categories.map((category) => (
      <div
        key={category.name}
        className="w-full max-w-[160px] flex items-center justify-center space-x-3 p-4 rounded-full bg-gray-100 shadow-sm cursor-pointer hover:bg-gray-200 transition-colors duration-200 border border-gray-300"
      >
