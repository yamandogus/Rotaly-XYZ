  ...
  const handleEditClick = (section: "personal" | "address") => {
    const data =
      section === "personal"
        ? personalInfo
            .filter((f) => f.id !== "role")
            .map((item) => ({ ...item })) // **Burada derin kopya yapıldı**
        : addressInfo.map((item) => ({ ...item })); // **Burada da**
  ...
  const validate = () => {
    ...
    tempData.forEach((field) => {
      ...
      if (!val) {
        ...
        newErrors[field.id] =
          "Geçerli telefon: +90 5XX XXX XXXX formatında olmalı";
      }
    });
    ...
    <div className="min-h-screen p-6 md:p-10 max-w-5xl mx-auto space-y-10">
      ...
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-md p-6 flex flex-col sm:flex-row items-center sm:items-start gap-6">
        ...
        <div className="text-center sm:text-left">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {getFieldValue(addressInfo, "city")}, {getFieldValue(addressInfo, "country")}
          </p>
        </div>
      </div>
      ...
      <Dialog
        ...
        onOpenChange={(isOpen) => {
          if (!isOpen) {
            // Diyalog kapandığında değişiklikleri iptal et
            setTempData([]);
            setErrors({});
            setEditSection(null);
          }
